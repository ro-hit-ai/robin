...
import * as Dialog from "@radix-ui/react-dialog"; // <-- fixed: import full Dialog namespace
...
// styling variant helper (kept simple — mirrors original intent)
const sidebarMenuButtonVariants = ({ variant = "default", size = "default" }) => {
  const baseClasses =
...
const SidebarProvider = React.forwardRef(
  (
    ...
    // init from localStorage (safe)
    const [_open, _setOpen] = React.useState(() => {
      try {
        const storedValue = typeof window !== "undefined" ? localStorage.getItem(SIDEBAR_STORAGE_KEY) : null;
    ...
    const setOpen = React.useCallback(
      (value) => {
        ...
        _setOpen(newValue);
        try {
          if (typeof window !== "undefined") localStorage.setItem(SIDEBAR_STORAGE_KEY, JSON.stringify(newValue));
        } catch (e) {
...
const Sidebar = React.forwardRef(
  ({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
    ...
    // Mobile: use Radix Dialog (Dialog.Root/Dialog.Portal/Dialog.Overlay/Dialog.Content)
    if (isMobile) {
      return (
        <Dialog.Root open={openMobile} onOpenChange={setOpenMobile}>
          <Dialog.Portal>
            ...
            <Dialog.Content
              ...
              data-mobile="true"
              className="fixed inset-y-0 z-50 flex flex-col bg-sidebar p-0 text-sidebar-foreground [&>button]:hidden"
              style={{ width: SIDEBAR_WIDTH_MOBILE, [side]: 0 }}
            >
    ...
    // Desktop rendering (keeps original structure)
    return (
      <div
        ref={ref}
...
const SidebarMenuSub = React.forwardRef(({ className, ...props }, ref) => (
  <ul ref={ref} data-sidebar="menu-sub" className={cn("flex min-w-0 ml-3.5 translate-x-px flex-col gap-1 border-l border-sidebar-border pl-2.5 pr-0 py-0.5 group-data-[collapsible=icon]:hidden", className)} {...props} />
));
