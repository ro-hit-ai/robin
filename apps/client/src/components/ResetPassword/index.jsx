...
// Mock API function to simulate password reset
const mockPostData = async ({ password, userId }) => {
  console.log("Resetting password for user:", userId, "with password:", password);
  // Simulate success or failure based on a condition (e.g., userId exists)
...
function ResetPassword({ user }) {
  ...
  const postData = async () => {
    if (check === password && password.length > 3) {
      const res = await mockPostData({ password, userId: user?.id });
      if (res.success) {
        alert("Password Reset Successful: The password has been updated successfully.");
      } else {
        alert("Error: There was a problem with your request.");
      }
    } else {
      alert("Error: Passwords do not match or are too short.");
    }
  };
