...
function UpdateUserModal({ user, onUpdate }) {
    ...
    try {
      const response = await fetch(`/api/v1/auth/user/role`, {
        ...
        headers: {
          ...
          // Replace getCookie with a custom solution or prop
      ...
      if (res.success === true) {
        // Replace toast with alert or a toast library like react-toastify
        alert("Success: User updated successfully");
      } else {
        alert(`Error: ${res.message}`);
      }
      if (onUpdate) {
        onUpdate(); // Call the onUpdate callback to handle refresh or state update
      }
      setOpen(false);
    } catch (error) {
